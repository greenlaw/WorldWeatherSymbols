...
## simplify svg for compatibility with GeoTools/GeoServer

if [ $# -ne 1 ]; then
  cat <<END
Usage: `basename $0` <png|readmes|simplify>
  png: generate PNG versions of symbols
  readmes: generate README.md files for symbol directory indexes
  simplify: simplify svg files by removing metadata/namespace elements that present GeoTools/GeoServer compatibility issues
...
case $1 in
    ...
    do
      ...
      echo "Automated list of symbols in the $DIRECTORY_NAME directory\n" >> $README 

      echo "|     | Symbols |" >> $README
      echo "| --- |:-------:|" >> $README

      svg_dir_files=`find $svg_dir -type f -name "*.svg"|sort`
      ...
      do
        ...
        echo "| **Description** | $TITLE |" >> $README
      done
      echo "\n*This \`README.md\` automatically generated by [www_manage.sh](https://github.com/OGCMetOceanDWG/WorldWeatherSymbols/blob/master/scripts/wws_manage.sh)*" >> $README
    done
esac

case $1 in 
    "simplify" )
    echo "Simplifying .svg files for GeoTools/GeoServer compatibility"
    for svg in $SVG_FILES
    do
      awk -v RS='^$' '{sub(/\n[^\S\n]*<svg:metadata>.*<\/svg:metadata>\n/,"\n"); gsub(/svg:/,""); sub(/xmlns:.*xmlns:svg/,"xmlns")}1' $svg > tmp && mv tmp $svg
      status=$?
      if [ $status -ne 0 ]; then
        COUNTER=$((COUNTER+1))
      fi
